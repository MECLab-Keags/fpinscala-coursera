...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    val output = (from until until).map(x => 0f).toArray
    lineOfSight(input.drop(from - 1).take(until - from), output)
    output.max
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    def loop(index: Int, angle: Float) {
      if(index < until) {
        val x = input(index)
        val ang = Math.max(x/index, angle)
        output(index) = ang
        loop(index+1, ang)
      }
    }
