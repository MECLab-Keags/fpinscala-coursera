...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("paraCountChange should return 0 for money < 0") {
    def check(money: Int, coins: List[Int]) =
  ...
  test("paraCountChange should return 1 when money == 0") {
    def check(coins: List[Int]) =
  ...
  test("paraCountChange should return 0 for money > 0 and coins = List()") {
    def check(money: Int) =
  ...
  test("paraCountChange should work when there is only one coin") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("paraCountChange should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, combinedThreshold(money, coins)) == expected,
        s"countChange($money, $coins) should be $expected")
