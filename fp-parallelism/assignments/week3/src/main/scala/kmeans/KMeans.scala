...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsByMean = points.par.groupBy(p => findClosest(p, means))
    means.par.map(m => m -> pointsByMean.getOrElse(m, GenSeq())).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val classifications = classify(points, means)
    val updates = update(classifications, means);

    if (!converged(eta)(means, updates)) kMeans(points, updates, eta) else updates // your implementation need to be tail recursive
  }
}
