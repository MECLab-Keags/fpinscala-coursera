...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        init <- initial
        ns <- {
          init._1.legalNeighbors
        }
        if !(explored contains ns._1)
      } yield (ns._1, ns._2 :: init._2)
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(Stream((startBlock, Nil)), Set.empty) //neighborsWithHistory(startBlock, Nil)
